...
const fallbackServiceData: ServicesContent = {
    heroHeadline: "Designing the Future of Digital Reality.",
    heroSubheadline: "We transform complex challenges into elegant, high-performance software solutions. Experience the next generation of IT excellence.",
    heroButtonText: "Explore Our Services", 
    services: [
        { id: '1', title: 'Custom Web Ecosystems', description: 'Scalable, secure, and lightning-fast web applications built for enterprise dominance.', offerings: [] },
        { id: '2', title: 'Next-Gen Mobile Apps', description: 'Native and cross-platform mobile experiences that redefine user engagement.', offerings: [] },
        { id: '3', title: 'Immersive UI/UX Design', description: 'Award-winning interfaces that blend aesthetics with intuitive functionality.', offerings: [] },
        { id: '4', title: 'Enterprise Software', description: 'Bespoke software tailored to streamline your most complex business operations.', offerings: [] },
        { id: '5', title: 'AI & Data Intelligence', description: 'Leveraging artificial intelligence to drive actionable insights and automation.', offerings: [] },
        { id: '6', title: 'Cloud Infrastructure', description: 'Robust cloud architectures ensuring 99.99% uptime and global scalability.', offerings: [] },
    ],
};
const fallbackBlogPosts: BlogPreview[] = [
    { id: 'dummy1', title: 'No Insights Yet', slug: '#', coverImageURL: '', summary: 'Our tech wizards are currently brewing the next big update. Stay tuned for industry-shaping insights.', createdAt: null },
];
const fallbackPortfolio: PortfolioPreview[] = [
    { id: 'p1', title: 'Quantum E-commerce', category: 'Web Platform', coverImageURL: '', content: 'A revolutionary e-commerce platform handling millions of transactions.' },
    { id: 'p2', title: 'Aura FinTech Core', category: 'Banking Tech', coverImageURL: '', content: 'Next-generation banking infrastructure with military-grade security.' },
    { id: 'p3', title: 'Nexus SaaS Portal', category: 'Enterprise', coverImageURL: '', content: 'A unified dashboard for managing global supply chains in real-time.' },
];
...
const fetchBlogPosts = async (): Promise<BlogPreview[]> => {
    try {
        ...
        return querySnapshot.docs.map(doc => {
            const data = doc.data();
            return { id: doc.id, title: data.title, slug: data.slug, coverImageURL: data.coverImageURL || '', summary: data.summary || data.content?.substring(0, 100).replace(/<\/?[^>]+(>|$)/g, "") + '...', createdAt: data.createdAt || null, } as BlogPreview;
        });
    } catch (error) { return fallbackBlogPosts; }
};
const fetchPortfolioProjects = async (): Promise<PortfolioPreview[]> => {
    try {
        const q = query(collection(db, 'portfolio'), orderBy('createdAt', 'desc'), limit(5)); 
        const querySnapshot = await getDocs(q);
        if (querySnapshot.empty) return fallbackPortfolio;
        return querySnapshot.docs.map(doc => {
            const data = doc.data();
            const cleanContent = data.content ? data.content.replace(/<[^>]*>?/gm, '').replace(/&nbsp;/g, ' ').substring(0, 80) + '...' : 'Innovation showcased.';
            return { id: doc.id, title: data.title, category: data.category || 'Technology', content: cleanContent, coverImageURL: data.coverImageURL || '', } as PortfolioPreview;
        });
...
const fetchLatestReviewsAndStats = async (): Promise<{ reviews: Review[], avgRating: number, totalCount: number }> => {
    try {
        ...
        const latestReviews = snapshotLatest.docs.map(doc => {
            const data = doc.data();
            return { id: doc.id, userName: data.userName || 'Anonymous Client', rating: data.rating || 5, comment: data.comment || 'Exceptional service and delivery.', createdAt: data.createdAt as Timestamp, } as Review;
        });
    ...
    return (
        <main className={styles.main}>
            ...
            <section className={styles.testimonialsSection}>
                ...
                {isLoading ? (<div style={{ textAlign: 'center', padding: '2rem' }}>Loading reviews...</div>) : (<><TestimonialSlider reviews={latestReviews} /><div className={styles.testimonialActions}><button onClick={openReviewModal} className={`${styles.heroButton} ${styles.primaryOutline}`} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><FaStar /> Rate Your Experience</button><Link href="/reviews" className={`${styles.heroButton} ${styles.secondary}`}>Read All Reviews</Link></div></>)}
            </section>
